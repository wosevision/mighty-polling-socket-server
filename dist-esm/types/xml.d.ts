 ...
 * }
 */
export declare module XML {
    /**
     * Add metadata to node under `$` property.
     */
    interface WithMetadata<M> {
        $: M;
    }
    /**
     ...
     */
    type TextWithMetadata<T extends string, M> = WithText<T> & WithMetadata<M>;
    /**
     * Join text and content into single node
     */
    type TextWithContent<T extends string, C> = WithText<T> & WithContent<C>;
    /**
     * Join content and metadata into single node.
     */
    type ContentWithMetadata<C, M> = WithContent<C> & WithMetadata<M>;
    /**
     * Join content, text and metadata into single node.
     */
    type TextWithContentAndMetadata<T extends string, C, M> = WithText<T> & WithContent<C> & WithMetadata<M>;
}
